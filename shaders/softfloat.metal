...
//
// The kernels below are used by this crate's own GPU-vs-CPU cross-checks
// and the `op_throughput_bench` benchmark suite. They are *not* part of
// the redistributable public API, so they're guarded behind an opt-in
// preprocessor flag. To compile them in (e.g. when integrating the file
// into your own test harness), pass `-DMETAL_SOFTFLOAT_TESTS=1` to the
// Metal compiler. The default build skips them entirely — the file is
// the same whether shipped as `softfloat.metal` or `dist/softfloat64.metal`.
#ifdef METAL_SOFTFLOAT_TESTS

// Pull internals into scope so the kernels below can call
// softfloat_add / softfloat_sub / etc. unqualified.
using namespace softfloat64_internal;

struct Op2Input  { ulong a; ulong b; uint mode; uint _pad; };
struct Op1Input  { ulong a; uint mode; uint _pad; };
struct Op3Input  { ulong a; ulong b; ulong c; uint mode; uint _pad; };

kernel void test_fma(
    device const Op3Input* inputs [[buffer(0)]],
    device ulong*          out    [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    Op3Input in = inputs[gid];
    out[gid] = softfloat_fma(in.a, in.b, in.c, in.mode);
}

kernel void test_fadd(
    device const Op2Input* inputs [[buffer(0)]],
    device ulong*          out    [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    Op2Input in = inputs[gid];
    out[gid] = softfloat_add(in.a, in.b, in.mode);
}

kernel void test_fsub(
    device const Op2Input* inputs [[buffer(0)]],
    device ulong*          out    [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    Op2Input in = inputs[gid];
    out[gid] = softfloat_sub(in.a, in.b, in.mode);
}

kernel void test_fmul(
    device const Op2Input* inputs [[buffer(0)]],
    device ulong*          out    [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    Op2Input in = inputs[gid];
    out[gid] = softfloat_mul(in.a, in.b, in.mode);
}

kernel void test_fdiv(
    device const Op2Input* inputs [[buffer(0)]],
    device ulong*          out    [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    Op2Input in = inputs[gid];
    out[gid] = softfloat_div(in.a, in.b, in.mode);
}

kernel void test_fsqrt(
    device const Op1Input* inputs [[buffer(0)]],
    device ulong*          out    [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    Op1Input in = inputs[gid];
    out[gid] = softfloat_sqrt(in.a, in.mode);
}

// --- Conversion test kernels --------------------------------------------
//
// All inputs / outputs are stored as ulong on the host buffers. i64 args
// reuse Op1Input's `a` field via two's-complement reinterpret. f32 and i64
// outputs are bit-cast / sign-extended into the ulong output.

kernel void test_cvt_i64_to_f64(
    device const Op1Input* inputs [[buffer(0)]],
    device ulong*          out    [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    Op1Input in = inputs[gid];
    out[gid] = softfloat_cvt_i64_to_f64((long)in.a, in.mode);
}

kernel void test_cvt_u64_to_f64(
    device const Op1Input* inputs [[buffer(0)]],
    device ulong*          out    [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    Op1Input in = inputs[gid];
    out[gid] = softfloat_cvt_u64_to_f64(in.a, in.mode);
}

kernel void test_cvt_f64_to_i64(
    device const Op1Input* inputs [[buffer(0)]],
    device ulong*          out    [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    Op1Input in = inputs[gid];
    // Reinterpret the i64 result as ulong so the host buffer layout
    // matches every other test kernel.
    out[gid] = (ulong)softfloat_cvt_f64_to_i64(in.a, in.mode);
}

kernel void test_cvt_f32_to_f64(
    device const Op1Input* inputs [[buffer(0)]],
    device ulong*          out    [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    // f32→f64 is exact; mode field is ignored. Caller stores the f32 bit
    // pattern in the low 32 bits of `a`.
    Op1Input in = inputs[gid];
    out[gid] = softfloat_cvt_f32_to_f64((uint)in.a);
}

kernel void test_cvt_f64_to_f32(
    device const Op1Input* inputs [[buffer(0)]],
    device ulong*          out    [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    Op1Input in = inputs[gid];
    // u32 result zero-extended into the ulong slot.
    out[gid] = (ulong)softfloat_cvt_f64_to_f32(in.a, in.mode);
}

// --- Comparison test kernels --------------------------------------------
//
// All comparisons are mode-independent (the `mode` field of Op2Input is
// ignored). Output is 0 / 1 stored as ulong.

kernel void test_feq(
    device const Op2Input* inputs [[buffer(0)]],
    device ulong*          out    [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    Op2Input in = inputs[gid];
    out[gid] = softfloat_feq(in.a, in.b) ? 1UL : 0UL;
}

kernel void test_flt(
    device const Op2Input* inputs [[buffer(0)]],
    device ulong*          out    [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    Op2Input in = inputs[gid];
    out[gid] = softfloat_flt(in.a, in.b) ? 1UL : 0UL;
}

kernel void test_fle(
    device const Op2Input* inputs [[buffer(0)]],
    device ulong*          out    [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    Op2Input in = inputs[gid];
    out[gid] = softfloat_fle(in.a, in.b) ? 1UL : 0UL;
}

kernel void test_fgt(
    device const Op2Input* inputs [[buffer(0)]],
    device ulong*          out    [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    Op2Input in = inputs[gid];
    out[gid] = softfloat_fgt(in.a, in.b) ? 1UL : 0UL;
}

kernel void test_fge(
    device const Op2Input* inputs [[buffer(0)]],
    device ulong*          out    [[buffer(1)]],
    uint gid [[thread_position_in_grid]])
{
    Op2Input in = inputs[gid];
    out[gid] = softfloat_fge(in.a, in.b) ? 1UL : 0UL;
}


#endif // METAL_SOFTFLOAT_TESTS
